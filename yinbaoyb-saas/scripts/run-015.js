...
async function main() {
  console.log('🚀 Running 015 fix audit fkeys migration...');
  
  const filePath = path.join(__dirname, '..', 'supabase', 'migrations', '015_fix_audit_fkeys.sql');
  ...
  if (response.ok) {
    ...
    console.error('❌ Migration 015 failed:', errorText);
    process.exit(1);
  }
}
