...
export default function AgendaPage() {
  ...
  async function handleCreate(e: React.FormEvent) {
    ...
    setForm({ patient_search: "", patient_id: "", therapist_id: "", type: "individual", date: selectedDate, start_time: "09:00", end_time: "10:00", notes: "" });
    setSaving(false);
    loadData();
  }
              ...
              <div key={date} className={`rounded-xl border ${isT ? "border-indigo-300 bg-indigo-50/30" : "border-gray-200 bg-white"} min-h-[120px]`}>
                ...
                <div className="p-1.5 space-y-1">
                  {apts.filter(a => a.status !== "cancelada").slice(0, 3).map(apt => (
                    <div key={apt.id} className={`px-1.5 py-1 rounded text-[10px] border ${typeColors[apt.type] || "bg-gray-100 text-gray-700"}`} onClick={() => setEditApt(editApt === apt.id ? null : apt.id)}>
              ...
              return (
                <div key={i} className={`bg-white p-1.5 min-h-[80px] ${!isCurrentMonth ? "opacity-40" : ""} ${isT ? "ring-2 ring-indigo-400 ring-inset" : ""}`} onClick={() => { setSelectedDate(date); setView("day"); }}>
                  ...
                  {apts.filter(a => a.status !== "cancelada").slice(0, 2).map(apt => (
                    <div key={apt.id} className={`px-1 py-0.5 rounded text-[9px] mb-0.5 ${typeColors[apt.type]?.replace("border-", "bg-").split(" ")[0] || "bg-gray-100"}`}>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40" onClick={() => setEditApt(null)}>
            <div className="bg-white rounded-2xl shadow-xl p-6 w-full max-w-md mx-4" onClick={e => e.stopPropagation()}>
              ...
              <div className="space-y-3 mb-4">
                <div className="flex items-center gap-2">
                  <span className={`px-2 py-0.5 rounded text-xs font-medium border ${typeColors[apt.type] || "bg-gray-100"}`}>{typeLabels[apt.type] || apt.type}</span>
                ...
                {apt.notes && <p className="text-sm text-gray-600 bg-gray-50 p-2 rounded">{apt.notes}</p>}
              </div>
              {/* Status actions */}
